 ...
 * @param log_data Logger data for driver log message verification
 * @param num_of_sessions Number of sessions to create concurrently
 * @param is_concurrently True if concurrent; false otherwise
 */
void open_sessions(CassCluster* cluster, const SessionContainer& sessions, test_utils::LogData* log_data, unsigned int num_of_sessions, bool is_concurrently = true) {
  ...
  for (unsigned int n = 0; n < (SESSION_STRESS_NUMBER_OF_ITERATIONS * 20); ++n) {
    if (static_cast<unsigned int>(log_data->message_count) != num_of_sessions) {
      boost::this_thread::sleep_for(boost::chrono::milliseconds(100));
    } else {
      break;
    }
  }
  BOOST_TEST_MESSAGE("\t\tOpened: " << log_data->message_count);
}
 ...
 * @param is_concurrently True if concurrent; false otherwise
 */
void close_sessions(const SessionContainer& sessions, test_utils::LogData* log_data, unsigned int num_of_sessions, bool is_concurrently = true) {
  ...
  BOOST_TEST_MESSAGE("\t\tClosed: " << log_data->message_count);
}
 ...
 *
 */
BOOST_AUTO_TEST_CASE(session_stress)
{
  //Create a data object to be passed to the log callback
  boost::scoped_ptr<test_utils::LogData> log_data(new test_utils::LogData(SESSION_STRESS_OPENED_LOG_MESSAGE));

  {
    //Create a three node cluster
    test_utils::CassClusterPtr cluster(cass_cluster_new());
    cass_cluster_set_log_level(cluster.get(), CASS_LOG_DEBUG);
    cass_cluster_set_log_callback(cluster.get(), test_utils::count_message_log_callback, log_data.get());
    cass_cluster_set_queue_size_io(cluster.get(), 65536);
    ...
    for (unsigned int iterations = 0; iterations < SESSION_STRESS_NUMBER_OF_ITERATIONS; ++iterations) {
      ...
      open_sessions(cluster.get(), sessions, log_data.get(), SESSION_STRESS_NUMBER_OF_SESSIONS, false);
      BOOST_CHECK_EQUAL(log_data->message_count, SESSION_STRESS_NUMBER_OF_SESSIONS);
      ...
      log_data->reset(SESSION_STRESS_CLOSED_LOG_MESSAGE);
      close_sessions(sessions, log_data.get(), SESSION_STRESS_NUMBER_OF_SESSIONS, false);
      BOOST_CHECK_EQUAL(log_data->message_count, SESSION_STRESS_NUMBER_OF_SESSIONS);
    ...
    for (unsigned int iterations = 0; iterations < SESSION_STRESS_NUMBER_OF_ITERATIONS; ++iterations) {
      BOOST_TEST_MESSAGE("\tOpening " << SESSION_STRESS_NUMBER_OF_SESSIONS << " sessions");
      log_data->reset(SESSION_STRESS_OPENED_LOG_MESSAGE);
      open_sessions(cluster.get(), sessions, log_data.get(), SESSION_STRESS_NUMBER_OF_SESSIONS);
      BOOST_CHECK_EQUAL(log_data->message_count, SESSION_STRESS_NUMBER_OF_SESSIONS);
      BOOST_CHECK_EQUAL(sessions.count(), SESSION_STRESS_NUMBER_OF_SESSIONS);

      BOOST_TEST_MESSAGE("\tClosing " << SESSION_STRESS_NUMBER_OF_SESSIONS << " sessions");
      log_data->reset(SESSION_STRESS_CLOSED_LOG_MESSAGE);
      close_sessions(sessions, log_data.get(), SESSION_STRESS_NUMBER_OF_SESSIONS);
      BOOST_CHECK_EQUAL(log_data->message_count, SESSION_STRESS_NUMBER_OF_SESSIONS);
      BOOST_CHECK_EQUAL(sessions.count(), 0);
    }
    ...
    for (unsigned int n = 0; n < quarter_sessions; ++n) {
      ...
      log_data->reset(SESSION_STRESS_OPENED_LOG_MESSAGE);
      open_sessions(cluster.get(), sessions, log_data.get(), quarter_sessions, false);
      BOOST_CHECK_EQUAL(log_data->message_count, quarter_sessions);
      ...
      log_data->reset(SESSION_STRESS_CLOSED_LOG_MESSAGE);
      close_sessions(sessions, log_data.get(), quarter_sessions, false);
      BOOST_CHECK_EQUAL(log_data->message_count, quarter_sessions);
      BOOST_CHECK_EQUAL(sessions.count(), 0);
    }
    ...
    //Create sessions
    log_data->reset(SESSION_STRESS_OPENED_LOG_MESSAGE);
    open_sessions(cluster.get(), sessions, log_data.get(), (SESSION_STRESS_NUMBER_OF_SESSIONS / 4), false);
    BOOST_CHECK_EQUAL(log_data->message_count, (SESSION_STRESS_NUMBER_OF_SESSIONS / 4));
    ...
    //Close sessions
    log_data->reset(SESSION_STRESS_CLOSED_LOG_MESSAGE);
    close_sessions(sessions, log_data.get(), (SESSION_STRESS_NUMBER_OF_SESSIONS / 4), false);
    BOOST_CHECK_EQUAL(log_data->message_count, (SESSION_STRESS_NUMBER_OF_SESSIONS / 4));
    BOOST_CHECK_EQUAL(sessions.count(), 0);
  }
}
